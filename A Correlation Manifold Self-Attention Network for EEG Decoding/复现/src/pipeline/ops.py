...
所有函数签名: tensor → tensor

### 矩阵操作

**Off (去对角)**:
$$
\text{Off}(X) = X - \text{diag}(\text{diag}(X))
$$

**Sym (对称化)**:
$$
\text{Sym}(X) = \frac{X + X^\top}{2}
$$

**Tril (下三角向量化)**:
$$
\text{tril}(X) = \text{vec}(\{X_{ij} : i > j\}) \in \mathbb{R}^{n(n-1)/2}
$$
...
**L2 规范化**:
$$
\text{normalize}(x) = \frac{x}{\|x\|_2 + \epsilon}
$$

**标准化**:
$$
\text{standardize}(x) = \frac{x - \mu}{\sigma + \epsilon}
$$
"""
